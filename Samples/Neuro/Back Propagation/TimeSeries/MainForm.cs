...
namespace TimeSeries
{
	...
	public class MainForm : System.Windows.Forms.Form
	{
        ...
        private double      elasticity = 0.15;
		private int			windowSize = 5;
		private int			predictionSize = 1;
		private int			iterations = 1000;

        private double[] hiddenLayerSizes = { 2.0 };
        private bool[] hiddenLayerConstant = { false };

		private Thread		workerThread = null;
		private bool		needToStop = false;

		private double[,]	windowDelimiter = new double[2, 2] { { 0, 0 }, { 0, 0 } };
        private TextBox elasticityBox;
        private Label labelElasticity;
        private TextBox hiddenLayerBox;
    ...
    " window size";
		...
		private void UpdateSettings( )
		{
            ...
            elasticityBox.Text = elasticity.ToString();
            hiddenLayerBox.Text = HiddenLayerText();
        ...
        private string HiddenLayerText()
        {
            System.Text.StringBuilder s = new System.Text.StringBuilder();

            for (int i=0; i<hiddenLayerSizes.Length; ++i)
            {
                if (!hiddenLayerConstant[i])
                {
                    s.Append('*');
                }

                s.Append(hiddenLayerSizes[i].ToString());
                s.Append(' ');
            }
        ...
        private void loadDataButton_Click(object sender, System.EventArgs e)
		{
			...
			if ( openFileDialog.ShowDialog( ) == DialogResult.OK )
			{
                ...
                // read maximum MAXREAD points
                double[] tempData = new double[MAXREAD];
				...
				try
				{
					...
					// read the data
					while ( ( i < MAXREAD) && ( ( str = reader.ReadLine( ) ) != null ) )
					{
						// parse the value
		...
		private void EnableControls( bool enable )
		{
            ...
            elasticityBox.Enabled = enable;
            hiddenLayerBox.Enabled = enable;
		...
		private void UpdateWindowSize( )
		{
			if ( data != null )
			{
				...
				try
				{
					windowSize = Math.Max( 1, Math.Min( 10000, int.Parse( windowSizeBox.Text ) ) );
				}
				catch
				{
					windowSize = 5;
        ...
        private void GetHiddenLayerText()
        {
            try
            {
                ParseHiddenLayerText(hiddenLayerBox.Text);
            }
            catch
            {
                ParseHiddenLayerText("*2.0");
            }
        }

        private void ParseHiddenLayerText(string s)
        {
            ...
            if (numLayers < 1)
            {
                throw new TypeLoadException("Empty hidden layer text");
            }

            hiddenLayerSizes = new double[numLayers];
            hiddenLayerConstant = new bool[numLayers];
            ...
            for (int i = 0; i < numLayers; ++i)
            {
                ...
                if (value.Contains("*"))
                {
                    hiddenLayerConstant[i] = false;
                    hiddenLayerSizes[i] = double.Parse(value.Replace('*', ' '));
                }
                else
                {
                    hiddenLayerConstant[i] = true;
                    hiddenLayerSizes[i] = double.Parse(value);
                }
            }
        }
        ...
        private void startButton_Click( object sender, System.EventArgs e )
		{
            ...
            try
            {
                elasticity = Math.Max(0, Math.Min( 1, double.Parse(elasticityBox.Text)));
            }
            catch
            {
                elasticity = 0.15;
		...
		void SearchSolution( )
		{
            ...
            double outputFactor = 1.0 - elasticity;
			...
			for ( int i = 0; i < samples; i++ )
			{
				...
				{
					input[i][j] = ( data[i + j] - yMin ) * factor - outputFactor;
				}
				// set output
				output[i][0] = ( data[i + windowSize] - yMin ) * factor - outputFactor;
			}

			// create multi-layer neural network
			ActivationNetwork	network = new ActivationNetwork(
				new BipolarSigmoidFunction( sigmoidAlphaValue ),
				windowSize,  // num inputs
                windowSize * 2, // 1st layer
                windowSize * 2, // 2nd layer
                1 ); // last layer (output?)
